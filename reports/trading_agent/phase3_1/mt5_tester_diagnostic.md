...
The tester sanity run was attempted three times. The third attempt used the
...
mode, `UseLocal=1`, and `ShutdownTerminal=1`. Three attempts produced no report;
the latest launch used `outputs/build/phase3-canonical-sanity.ini` after the
...
The latest attempt added the expected Login/Server and live-trading-disabled
sections but logged the same failure sequence in `logs/20260916.log`:
